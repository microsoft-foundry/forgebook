#!/usr/bin/env python3
"""Generate or edit images via Microsoft Foundry (Azure AI Projects SDK).

Defaults to gpt-image-1.5 and a structured prompt augmentation workflow.
Uses PROJECT_ENDPOINT + API_KEY to authenticate through Azure AI Foundry.
"""
...
def _ensure_env(dry_run: bool) -> None:
    ...
    if endpoint and api_key:
        print("PROJECT_ENDPOINT and API_KEY are set.", file=sys.stderr)
        return
    if dry_run:
        missing = []
        if not endpoint:
    ...
    _die(
        ...
        "Set these environment variables or add them to a .env file in the repo root."
    )


def _load_dotenv() -> None:
    """Best-effort .env loading from repo root."""
    try:
...
def _create_client():
    """Create an OpenAI client via Azure AI Projects SDK."""
    try:
        ...
        _die(
            ...
            "Install with `uv pip install azure-ai-projects azure-identity`."
        )
...
def _create_async_client():
    """Create an async OpenAI client via Azure AI Projects SDK."""
    try:
...
def _generate(args: argparse.Namespace) -> None:
    ...
    print(
        "Calling Image API (generation) via Azure AI Foundry. This can take up to a couple of minutes.",
        file=sys.stderr,
    )
    started = time.time()
...
def _edit(args: argparse.Namespace) -> None:
    ...
    print(
        f"Calling Image API (edit) via Azure AI Foundry with {len(image_paths)} image(s).",
        file=sys.stderr,
    )
    started = time.time()
    client = _create_client()
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Generate or edit images via Azure AI Foundry Image API"
    )
