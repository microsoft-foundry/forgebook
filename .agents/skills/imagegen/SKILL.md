---
name: "imagegen"
description: "Use when the user asks to generate or edit images via the Azure AI Foundry Image API (for example: generate image, edit/inpaint/mask, background removal or replacement, transparent background, product shots, concept art, covers, or batch variants); run the bundled CLI (`scripts/image_gen.py`) and require `PROJECT_ENDPOINT` + `API_KEY` for live calls."
---


# Image Generation Skill

Generates or edits images for the current project (e.g., website assets, game assets, UI mockups, product mockups, wireframes, logo design, photorealistic images, infographics). Defaults to `gpt-image-1.5` via Microsoft Foundry (Azure AI Projects SDK), and prefers the bundled CLI for deterministic, reproducible runs.
...
Python packages:
```
uv pip install azure-ai-projects azure-identity openai pillow
```
If `uv` is unavailable:
```
python3 -m pip install azure-ai-projects azure-identity openai pillow
```

## Environment
- `PROJECT_ENDPOINT` must be set to your Azure AI Foundry project endpoint (e.g., `https://<resource>.services.ai.azure.com/api/projects/<project>`).
- `API_KEY` must be set for live API calls (Azure AI Foundry API key).

If the variables are missing, give the user these steps:
1. Go to your Azure AI Foundry project in the Azure portal.
2. Copy the project endpoint and API key from the project settings.
3. Set `PROJECT_ENDPOINT` and `API_KEY` as environment variables, or add them to a `.env` file in the repo root.
- Never ask the user to paste credentials in chat. Ask them to set them locally and confirm when ready.

If installation isn't possible in this environment, tell the user which dependency is missing and how to install it locally.

## Defaults & rules
- Use `gpt-image-1.5` unless the user explicitly asks for a different model.
- Assume the user wants a new image unless they explicitly ask for an edit.
- Require `PROJECT_ENDPOINT` and `API_KEY` before any live API call.
- Use the Azure AI Projects SDK (`azure.ai.projects`) to obtain an OpenAI-compatible client for all API calls; do not use raw HTTP.
