![Toolboxes in Microsoft Foundry, three pillars available today. Pillar 1, Build & curate: select tools, configure their authentication centrally, and publish a reusable, versioned toolbox. The toolbox (default version - immutable and promotable) holds the curated tools (Web Search, MCP, Azure AI Search, Code Interpreter, A2A, Skills, and more) plus pillar 2, Discover with Tool Search (two meta-tools, search on demand, progressive disclosure), and pillar 3, Govern (an RAI guardrail over all tools and a bring-your-own AI gateway for MCP servers, governed by default with more to come). The toolbox is exposed through a single MCP-compatible endpoint /toolboxes/{name}/mcp, consumed unchanged by any MCP client - here Microsoft Agent Framework, LangGraph, and the Copilot SDK.](media/mastering-foundry-toolbox/01-toolbox-architecture.svg)
