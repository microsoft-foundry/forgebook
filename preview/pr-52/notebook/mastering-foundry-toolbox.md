![Toolboxes in Microsoft Foundry, shown left to right. Build & curate (left): select tools (Web Search, MCP, Azure AI Search, Code Interpreter, A2A, Skills, and more), configure their authentication centrally, and publish a reusable, versioned toolbox. The toolbox (center, default version, governed by default) holds the curated tools, adds Discover with Tool Search (two meta-tools, search on demand, progressive disclosure), and is exposed through a single MCP-compatible endpoint /toolboxes/{name}/mcp. Consume (right): any MCP client connects to the same URL unchanged - here Microsoft Agent Framework, LangGraph, and the Copilot SDK.](media/mastering-foundry-toolbox/01-toolbox-architecture.svg)
